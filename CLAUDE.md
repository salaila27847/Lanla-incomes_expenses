...
`controller/src/index.ts` imports `express-async-errors` before any router and ends the middleware chain with a catch-all error handler. Express 4 (what this app runs) does not forward a rejected promise inside an `async (req, res) => {...}` route handler to error middleware on its own — without this, a route whose Sheets write throws (most commonly: the real Sheet is missing a tab a newer feature added, since writes deliberately don't tolerate that the way `readOptionalRange` does for reads) leaves the request hanging forever with no response, which from the PWA looks exactly like the button doing nothing. This bit once: `/qr/pending/:id/deduct` silently hung in production because the user's Sheet predated the `SavingsWithdrawals` tab.
...
The dashboard's two balances come from different places on purpose: the **spending** column is derived — income − expense for that cycle alone, no carry-forward from `opening_balance` or from any other cycle — while the **savings** account is entered by hand per cycle, because money sometimes leaves it directly — which is what the SavingsQR transfer-back flow exists to reverse. (An earlier version ran the spending figure forward as a cumulative balance across every cycle on record; that made it diverge from "this cycle's income minus this cycle's expense" the moment more than one cycle existed, so it was dropped in favor of a plain per-cycle net. `Settings.opening_balance` still exists and is still editable, but nothing on the dashboard reads it any more.) A savings-tagged `Income` entry is the one exception that writes to `Cycles.SavingsBalance` automatically rather than waiting for a hand-typed correction — see "Savings deposits" below for why that's safe.
...
The `PriceHistory` row a settlement writes carries `fundedBySavings` (`sheets/client.ts`), set only by the `/deduct` path — `/confirm` writes `false`, same as every normal row. Both settlements still write the same shape of `PriceHistory` row (so `/prices` treats them identically — see above), but `budget.ts`'s `spentThisCycle` and `dashboard.ts`'s food/goods totals skip a `fundedBySavings` row. Without that exclusion a `/deduct`-settled line would be deducted twice over: once from `Cycles.SavingsBalance` via the `SavingsWithdrawal` above, and again from the spending account's own food/goods cap and net — an account that never paid for it. `/confirm`-settled lines get no such exclusion, correctly: the spending account really does end up paying those back.
...
Every row in `SavingsQR`'s movement list is tappable, the same interaction as Budget's expense list: an "in" row edits/deletes the underlying `Income` entry (`PUT`/`DELETE /income/:id`), an "out" row the underlying `SavingsWithdrawal` (`PUT`/`DELETE /qr/withdrawals/:id`), and only the amount is editable on either — same scope as Budget's own inline editor, which doesn't offer to rename the item either. Both routes correct `Cycles.SavingsBalance` by the amount's delta rather than leaving it stuck at whatever the original entry folded in, for the same reason the original write did: the app was just told about a fully-known change. Deleting a withdrawal only reverses its own bookkeeping (the balance credit, the row itself) — it does not touch the `PriceHistory` row the `/deduct` settlement already wrote, since price history doesn't care which account ended up paying.
