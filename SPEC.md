...
> - **Backend (OCR / fuzzy matching / QR generation):** Python + FastAPI on Vercel, as specified. OCR uses a cloud API (Google Cloud Vision — Document Text Detection) rather than a self-hosted model, since Vercel serverless functions have tight package-size and cold-start budgets that a bundled OCR model would blow through — this keeps the spec's own "no cold start, fast latency" goal intact. [Typhoon OCR](https://opentyphoon.ai/) (Thai-specific) is worth evaluating as a swap-in if Vision API's line-item segmentation on Thai receipts proves weak. Fuzzy matching uses `rapidfuzz`. QR generation builds the PromptPay EMVCo payload and renders it with the `qrcode` package.
