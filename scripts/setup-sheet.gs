 ...
 *
 * If your PriceHistory tab predates the FundedBySavings column, add it by
 * hand (see SETUP.md), then run `backfillFundedBySavings` once: it matches
 * existing SavingsWithdrawals rows back to the PriceHistory rows they came
 * from and marks them, so past "หักจากบัญชีเงินออม" purchases stop being
 * deducted from the spending account's own budget too.
 */
...
/**
 * One-time backfill for PriceHistory rows written before the
 * FundedBySavings column existed: a purchase already settled as a
 * permanent savings drawdown ("หักจากบัญชีเงินออม") whose PriceHistory row
 * still reads as an ordinary spending-account expense on Budget and
 * Dashboard, because nothing marked it otherwise at the time.
 *
 * Run once, after adding the FundedBySavings column to an existing
 * PriceHistory tab (see SETUP.md). Safe to run more than once: a row
 * already marked TRUE is left alone, not re-matched.
 *
 * There is no ID linking a SavingsWithdrawals row to the PriceHistory row
 * the same /deduct call wrote -- they were two separate appends -- so this
 * matches them by Date + MasterItemName + Category + amount (Price *
 * Quantity - Discount, the same arithmetic as the app's lineTotal). A
 * withdrawal that matches more than one unmarked PriceHistory row, or
 * none at all, is left untouched and listed at the end instead of
 * guessed at: picking the wrong row would wrongly hide a real
 * spending-account purchase from the budget, which is worse than leaving
 * a handful for you to mark by hand.
 */
function backfillFundedBySavings() {
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var withdrawalsSheet = spreadsheet.getSheetByName('SavingsWithdrawals');
  var priceHistorySheet = spreadsheet.getSheetByName('PriceHistory');

  if (!withdrawalsSheet || !priceHistorySheet) {
    var missingMsg = 'Need both the SavingsWithdrawals and PriceHistory tabs to exist first.';
    Logger.log(missingMsg);
    SpreadsheetApp.getUi().alert(missingMsg);
    return;
  }

  var withdrawalRows = withdrawalsSheet.getLastRow() > 1
    ? withdrawalsSheet.getRange(2, 1, withdrawalsSheet.getLastRow() - 1, 6).getValues()
    : [];
  var priceHistoryRows = priceHistorySheet.getLastRow() > 1
    ? priceHistorySheet.getRange(2, 1, priceHistorySheet.getLastRow() - 1, 9).getValues()
    : [];

  // Mirrors client.ts's lineTotal(): blank Quantity reads as 1, blank
  // Discount as 0, so rows from before those columns existed still total
  // correctly.
  function lineTotal(row) {
    var price = Number(row[4]) || 0;
    var quantity = row[5] === '' || row[5] === null ? 1 : Number(row[5]);
    var discount = row[7] === '' || row[7] === null ? 0 : Number(row[7]);
  ...
  withdrawalRows.forEach(function (withdrawal) {
    var date = withdrawal[1];
    var masterItemName = withdrawal[2];
    var category = withdrawal[3];
    var amount = Number(withdrawal[4]) || 0;
    var label = masterItemName + ' (' + date + ', ' + amount + ')';

    var candidates = [];
    priceHistoryRows.forEach(function (row, rIndex) {
      if (String(row[0]) !== String(date)) return;
      if (String(row[2]) !== String(masterItemName)) return;
      if (String(row[3]) !== String(category)) return;
      if (Math.abs(lineTotal(row) - amount) > 0.01) return; // satang-level noise only
      candidates.push(rIndex);
    });

    var unmarked = candidates.filter(function (rIndex) {
      return priceHistoryRows[rIndex][8] !== 'TRUE';
    });

    if (candidates.length && unmarked.length === 0) {
      alreadyMarked += 1; // every match for this withdrawal is already TRUE
    } else if (unmarked.length === 0) {
      unmatched.push(label);
    } else if (unmarked.length > 1) {
      ambiguous.push(label + ' -- ' + unmarked.length + ' matching rows');
    } else {
      var rIndex = unmarked[0];
      priceHistorySheet.getRange(rIndex + 2, 9).setValue('TRUE');
      priceHistoryRows[rIndex][8] = 'TRUE'; // keep this run's own copy in step
      updated += 1;
    }
  });

  var summary =
    'Marked FundedBySavings = TRUE on ' + updated + ' row(s).\n' +
    'Already marked from an earlier run: ' + alreadyMarked + '\n' +
    'No matching PriceHistory row found: ' + (unmatched.length || '(none)') +
    (unmatched.length ? '\n  - ' + unmatched.join('\n  - ') : '') + '\n' +
    'Ambiguous, left untouched: ' + (ambiguous.length || '(none)') +
    (ambiguous.length ? '\n  - ' + ambiguous.join('\n  - ') : '') +
    (unmatched.length || ambiguous.length
      ? '\n\nFor anything listed above, find the row yourself on PriceHistory and ' +
        'set its FundedBySavings cell (column I) to TRUE by hand.'
      : '');
